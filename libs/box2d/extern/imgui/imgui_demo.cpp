...
// be reentrant or used in threads. This might be a pattern you will want to use in your code, but
// most of the real data you would be editing is likely going to be stored outside your functions.
...
#pragma clang diagnostic ignored \
        "-Wdeprecated-declarations"  // warning : 'xx' is deprecated: The POSIX name for this item..
                                     // // for strdup used in demo code (so user can copy & paste
                                     // the code)
...
#if (__GNUC__ >= 6)
#pragma GCC diagnostic ignored \
        "-Wmisleading-indentation"  // warning: this 'if' clause does not guard this statement //
...
void ImGui::ShowDemoWindow(bool* p_open) {
    ...
    if (show_app_documents)
        ShowExampleAppDocuments(&show_app_documents);  // Process the Document app next, as it may
    ...
    if (ImGui::CollapsingHeader("Configuration")) {
        ...
        if (ImGui::TreeNode("Configuration##2")) {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Required back-end to feed in gamepad inputs in io.NavInputs[] and set "
            ...
            ImGui::SameLine();
            ShowHelpMarker("Instruct navigation to move the mouse cursor. See comment for "
                           "ImGuiConfigFlags_NavEnableSetMousePos.");
            ...
            ImGui::SameLine();
            ShowHelpMarker("Set to false to disable blinking cursor, for users who consider it "
                           "distracting");
            ...
            ImGui::SameLine();
            ShowHelpMarker(
                    "Instruct Dear ImGui to render a mouse cursor for you. Note that a mouse "
                    "cursor rendered via your application GPU rendering path will feel more laggy "
                    "than hardware cursor, but will be more in sync with your other "
        ...
        if (ImGui::TreeNode("Capture/Logging")) {
            ImGui::TextWrapped(
                    ...
                    "content of a window or a block. Tree nodes can be automatically expanded.");
            ShowHelpMarker("Try opening any of the contents below in this window and then click "
                           "one of the \"Log To\" button.");
            ImGui::LogButtons();
            ImGui::TextWrapped("You can also call ImGui::LogText() to output directly to the log "
...
static void ShowDemoWindowWidgets() {
    ...
    if (ImGui::TreeNode("Basic")) {
...

        {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Refer to the \"Combo\" section below for an explanation of the full "
...

        {
            ...
            ShowHelpMarker("USER:\nHold SHIFT or use mouse to select text.\n"
                           ...
                           "InputText() to a dynamic string type. See misc/cpp/imgui_stdlib.h for "
                           "an example (this is not demonstrated in imgui_demo.cpp).");

            ImGui::InputInt("input int", &i0);
            ImGui::SameLine();
            ShowHelpMarker("You can apply arithmetic operators +,*,/ on numerical values.\n  e.g. "
                           "[ 100 ], input \'*2\', result becomes [ 200 ]\nUse +- to subtract.\n");
    ...
    if (ImGui::TreeNode("Trees")) {
        ...
        if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
            ...
            static int selection_mask =
                    (1 << 2);  // Dumb representation of what may be user-side selection state. You
                               // may carry selection state inside or outside your objects in
                               // whatever format you see fit.
            int node_clicked =
                    -1;  // Temporary storage of what node we have clicked to process selection at
                         // the end of the loop. May be a pointer to your own node type, etc.
            ImGui::PushStyleVar(
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (i < 3) {
                    ...
                    // Leaf: The only reason we have a TreeNode at all is to allow selection of the
                    // leaf. Otherwise we can use BulletText() or TreeAdvanceToLabelPos()+Text().
    ...
    if (ImGui::TreeNode("Images")) {
        ...
        ImGui::TextWrapped(
                ...
                "access to in this demo). Use the 'ImTextureID' type as storage to pass pointers "
                "or identifier to your own texture data. Hover the texture for a zoomed view!");
    ...
    if (ImGui::TreeNode("Combo")) {
        ...
        if (ImGui::CheckboxFlags("ImGuiComboFlags_NoPreview", (unsigned int*)&flags,
            ...
            flags &= ~ImGuiComboFlags_NoArrowButton;  // Clear the other flag, as we cannot combine
                                                      // both
        ...
        if (ImGui::BeginCombo("combo 1", item_current,
                              flags))  // The second parameter is the label previewed before opening
                                       // the combo.
        {
    ...
    if (ImGui::TreeNode("Color/Picker Widgets")) {
        ...
        ImGui::SameLine();
        ShowHelpMarker("SetColorEditOptions() is designed to allow you to set boot-time "
                       "default.\nWe don't have Push/Pop functions because you can force options "
                       "on a per-widget basis if needed, and the user can change non-forced ones "
...
// hence the /2 below.
        ...
        ImGui::SameLine();
        ShowHelpMarker("As with every widgets in dear imgui, we never modify values unless there "
        ...
        ImGui::DragScalar("drag float ^2", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero, &f32_one,
                          "%f", 2.0f);
        ImGui::SameLine();
        ShowHelpMarker("You can use the 'power' parameter to increase tweaking precision on one "
                       "side of the range.");
...
static void ShowDemoWindowLayout() {
    ...
    if (ImGui::TreeNode("Child windows")) {
        ShowHelpMarker("Use child windows to begin into a self-contained independent "
    ...
    if (ImGui::TreeNode("Tabs")) {
        ...
        if (ImGui::TreeNode("Advanced & Close Button")) {
            ...
            ImGui::CheckboxFlags("ImGuiTabBarFlags_AutoSelectNewTabs",
                                 (unsigned int*)&tab_bar_flags, ImGuiTabBarFlags_AutoSelectNewTabs);
    ...
    if (ImGui::TreeNode("Groups")) {
        ShowHelpMarker("Using ImGui::BeginGroup()/EndGroup() to layout items. BeginGroup() "
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
        ShowHelpMarker("This is testing the vertical alignment that gets applied on text to keep "
                       "it aligned with widgets. Lines only composed of text or \"small\" widgets "
                       "fit in less vertical spaces than lines with normal widgets.");
        ...
        bool node_open = ImGui::TreeNode(
                "Node##2");  // Common mistake to avoid: if we want to SameLine after TreeNode we
    ...
    if (ImGui::TreeNode("Clipping")) {
        ...
        ImGui::TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it "
                           "won't fit in its frame. Otherwise we are doing coarser clipping + "
                           "passing a scissor rectangle to the renderer. The system is designed to "
...
static void ShowDemoWindowPopups() {
    ...
    if (ImGui::TreeNode("Modals")) {
        ...
        if (ImGui::BeginPopupModal("Stacked 1")) {
            ...
            ImGui::ColorEdit4(
                    "color",
                    color);  // This is to test behavior of stacked regular popups over a modal
...
static void ShowDemoWindowColumns() {
    ...
    */
    ...
    ImGui::SameLine();
    ShowHelpMarker("NB: Tree node must be poped before ending the cell. There's no storage of "
                   "state per-cell.");
    if (node_open) {
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
        ...
        if (ImGui::TreeNode("Dragging")) {
            ...
            if (ImGui::IsItemActive()) {
                ...
                // (the default threshold is stored in io.MouseDragThreshold) You can request a
                // lower or higher threshold using the second parameter of IsMouseDragging() and
                // GetMouseDragDelta()
        ...
        if (ImGui::TreeNode("Mouse cursors")) {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Your application can render a different mouse cursor based on what "
                           "ImGui::GetMouseCursor() returns. If software cursor rendering "
                           "(io.MouseDrawCursor) is set ImGui will draw the right cursor for you, "
...
void ImGui::ShowFontSelector(const char* label) {
    ...
    ShowHelpMarker("- Load additional fonts with io.Fonts->AddFontFromFileTTF().\n"
                   ...
                   "- If you need to add/remove fonts at runtime (e.g. for DPI change), do it "
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
    ...
    if (ImGui::Button("Revert Ref"))
        style = *ref;
    ImGui::SameLine();
    ShowHelpMarker("Save/Revert in local non-persistent storage. Default Colors definition are not "
                   "affected. Use \"Export Colors\" below to save them somewhere.");
    ...
    if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
        if (ImGui::BeginTabItem("Sizes")) {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Adjust if you cannot see the edges of your screen (e.g. on a TV where "
            ...
            for (int i = 0; i < atlas->Fonts.Size; i++) {
                ...
                if (font_details_opened) {
                    ...
                    ShowHelpMarker(
                            ...
                            "scaled.\n\nFont are currently rendered into bitmaps at a given size "
                            "at the time of building the atlas. You may oversample them to get "
                            "some flexibility with scaling. You can also render at multiple sizes "
                            "and select which one to use at runtime.\n\n(Glimmer of hope: the "
                            "atlas system should hopefully be rewritten in the future to make "
        ...
        if (ImGui::BeginTabItem("Rendering")) {
            ...
            ImGui::SameLine();
            ShowHelpMarker("When disabling anti-aliasing lines, you'll probably want to disable "
...
static void ShowExampleAppLog(bool* p_open) {
    ...
    // We take advantage of the fact that multiple calls to Begin()/End() are appending to the same
    // window.
...
static void ShowExampleAppPropertyEditor(bool* p_open) {
    ...
    ShowHelpMarker("This example shows how you may implement a property editor using two "
                   ...
                   "simple cases, you can use ImGui::SameLine(xxx) to position\nyour cursor "
...
static void ShowExampleAppAutoResize(bool* p_open) {
    ...
    ImGui::Text("Window will resize every-frame to the size of its content.\nNote that you "
                "probably don't want to query the window size to\noutput your content because that "
...
static void ShowExampleAppCustomRendering(bool* p_open) {
    ...
    ImGui::ColorEdit3("Color", &col.x);
    {
        ...
        x += spacing +
             spacing;  // Vertical line (faster than AddLine, but only handle integer thickness)
    ...
    ImGui::Separator();
    {
        ...
        // allows us to use IsItemHovered() But you can also draw directly and poll mouse/keyboard
        // by yourself. You can manipulate the cursor using GetCursorPos() and SetCursorPos(). If
        // you only use the ImDrawList API, you can notify the owner window of its extends by using
        // SetCursorPos(max).
...
struct MyDocument {
    ...
    static void DisplayContents(MyDocument* doc) {
        ...
        if (ImGui::Button("Save", ImVec2(100, 0)))
            doc->DoSave();
        ImGui::ColorEdit3("color", &doc->Color.x);  // Useful to test drag and drop and
