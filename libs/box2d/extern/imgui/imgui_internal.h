...
typedef int ImGuiItemStatusFlags;    // -> enum ImGuiItemStatusFlags_   // Flags: for
                                     // DC.LastItemStatusFlags
typedef int ImGuiNavHighlightFlags;  // -> enum ImGuiNavHighlightFlags_ // Flags: for
                                     // RenderNavHighlight()
typedef int ImGuiNavDirSourceFlags;  // -> enum ImGuiNavDirSourceFlags_ // Flags: for
                                     // GetNavInputAmount2d()
typedef int ImGuiNavMoveFlags;  // -> enum ImGuiNavMoveFlags_      // Flags: for navigation requests
typedef int ImGuiSeparatorFlags;  // -> enum ImGuiSeparatorFlags_    // Flags: for Separator() -
                                  // internal
...
// signed/unsigned int/long long float/double, using templates here but we could also redefine them
// 6 times
template <typename T>
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_PressedOnClickRelease = 1 << 1,  // return true on click + release on same item
    ...
    ImGuiButtonFlags_AlignTextBaseLine =
            1 << 9,  // vertically align button to match text baseline - ButtonEx() only // FIXME:
                     // Should be removed and handled by SmallButton(), not possible currently
                     // because of DC.CursorPosPrevLine
    ImGuiButtonFlags_NoKeyModifiers = 1 << 10,     // disable interaction if a key modifier is held
    ImGuiButtonFlags_NoHoldingActiveID = 1 << 11,  // don't set ActiveId while holding the mouse
                                                   // (ImGuiButtonFlags_PressedOnClick only)
    ImGuiButtonFlags_PressedOnDragDropHold =
            1 << 12,  // press when held into while we are drag and dropping another item (used by
...
enum ImGuiColumnsFlags_ {
    ...
    ImGuiColumnsFlags_GrowParentContentsSize =
            1 << 4  // (WIP) Restore pre-1.51 behavior of extending the parent window contents size
                    // but _without affecting the columns width at all_. Will eventually remove.
};
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId =
            1 << 4,  // Allow scoring and considering the current NavId as a move target candidate.
...
struct IMGUI_API ImGuiInputTextState {
    ...
    ImVector<char>
            TempBuffer;  // temporary buffer for callback and other other operations. size=capacity.
...
struct ImGuiSettingsHandler {
    ...
    void* (*ReadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler,
                        const char* name);  // Read: Called when entering into a new ini entry e.g.
                                            // "[Window][Name]"
    void (*ReadLineFn)(
...
struct ImGuiPopupRef {
    ...
    ImGuiWindow* Window;  // Resolved on BeginPopup() - may stay unresolved if user never calls
                          // OpenPopup()
    ImGuiWindow* ParentWindow;  // Set on OpenPopup()
    int OpenFrameCount;         // Set on OpenPopup()
    ImGuiID OpenParentId;  // Set on OpenPopup(), we need this to differenciate multiple menu sets
...
struct ImGuiNextWindowData {
    ...
    ImGuiNextWindowData() {
        PosCond = SizeCond = ContentSizeCond = CollapsedCond = SizeConstraintCond = FocusCond =
                BgAlphaCond = 0;
...
struct ImGuiContext {
    ...
    bool ActiveIdHasBeenEdited;  // Was the value associated to the widget Edited over the course of
                                 // the Active state.
    ...
    ImGuiWindow*
            MovingWindow;  // Track the window we clicked on (in order to preserve focus). The
                           // actually window that is moved is generally MovingWindow->RootWindow.
    ...
    ImGuiWindow* NavWindowingTarget;  // When selecting a window (holding Menu+FocusPrev/Next, or
                                      // equivalent of CTRL-TAB) this window is temporarily
    ...
    float DimBgRatio;  // 0.0..1.0 animation when fading in a dimming background (for modal window
                       // and CTRL+TAB list)
    ImDrawList OverlayDrawList;  // Optional software render of mouse cursors, if io.MouseDrawCursor
                                 // is set + a few debug overlays
    ...
    ImVec2 ScrollbarClickDeltaToGrabCenter;  // Distance between mouse and center of grab box,
                                             // normalized in parent space. Use storage?
    int TooltipOverrideCount;
    ImVector<char> PrivateClipboard;  // If no custom clipboard handler is defined

    // Platform support
    ImVec2 PlatformImePos, PlatformImeLastPos;  // Cursor position request & last passed to the OS
                                                // Input Method Editor
    ...
    ImGuiTextBuffer LogClipboard;  // Accumulation buffer when log to clipboard. This is pointer so
                                   // our GImGui static constructor doesn't call heap allocators.
    int LogStartDepth;
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    ImVec2 CursorMaxPos;    // Used to implicitly calculate the size of our contents, always growing
                          // during the frame. Turned into window->SizeContents at the beginning of
                          // next frame
...
struct IMGUI_API ImGuiWindow {
    ...
    ImVec2 SizeFullAtLastBegin;  // Copy of SizeFull at the end of Begin. This is the reference
                                 // value we'll use on the next frame to decide if we need
                                 // scrollbars.
    ImVec2 SizeContents;  // Size of contents (== extents reach of the drawing cursor) from previous
                          // frame. Include decoration, window title, border, menu, etc.
    ImVec2 SizeContentsExplicit;  // Size of contents explicitly set by the user via
    ...
    ImVec2 ScrollTarget;  // target scroll position. stored as cursor position with scrolling
                          // canceled out, so the highest point is always 0.0f. (FLT_MAX for no
                          // change)
    ...
    int HiddenFramesForResize;  // Hide the window for N frames while allowing items to be submitted
                                // so we can measure their size
    ImGuiCond
            SetWindowPosAllowFlags;  // store acceptable condition flags for SetNextWindowPos() use.
    ImGuiCond SetWindowSizeAllowFlags;  // store acceptable condition flags for SetNextWindowSize()
                                        // use.
    ...
    ImRect ClipRect;  // Current clipping rectangle. = DrawList->clip_rect_stack.back(). Scissoring
                      // / clipping rectangle. x1, y1, x2, y2.
    ImRect OuterRectClipped;  // = WindowRect just after setup in Begin(). == window->Rect() for
                              // root window.
    ImRect InnerMainRect, InnerClipRect;
    ImRect ContentsRegionRect;  // FIXME: This is currently confusing/misleading. Maximum visible
                                // content position ~~ Pos + (SizeContentsExplicit ?
                                // SizeContentsExplicit : Size - ScrollbarSizes) - CursorStartPos,
                                // per axis
    ...
    ImGuiWindow* RootWindowForTitleBarHighlight;  // Point to ourself or first ancestor which will
                                                  // display TitleBgActive color when this window is
                                                  // active.
    ImGuiWindow* RootWindowForNav;  // Point to ourself or first ancestor which doesn't have the
                                    // NavFlattened flag.

    ImGuiWindow*
            NavLastChildNavWindow;  // When going to the menu bar, we remember the child window we
    ...
    // Navigation / Focus
    // FIXME-NAV: Merge all this with the new Nav system, at least the request variables should be
    // moved to ImGuiContext
    int FocusIdxAllCounter;         // Start at -1 and increase as assigned via FocusItemRegister()
    int FocusIdxTabCounter;         // (same, but only count widgets which you can Tab through)
    int FocusIdxAllRequestCurrent;  // Item being requested for focus
    int FocusIdxTabRequestCurrent;  // Tab-able item being requested for focus
    int FocusIdxAllRequestNext;     // Item being requested for focus, for next update (relies on
                                 // layout to be stable between the frame pressing TAB and the next
                                 // frame)
...
enum ImGuiTabBarFlagsPrivate_ {
    ...
    ImGuiTabBarFlags_DockNodeIsDockSpace =
            1 << 21,  // [Docking: Unused in Master Branch] Part of an explicit dockspace node node
    ImGuiTabBarFlags_IsFocused = 1 << 22,
    ImGuiTabBarFlags_SaveSettings =
            1 << 23  // FIXME: Settings are handled by the docking system, this only request the tab
...
IMGUI_API void Shutdown(ImGuiContext* context);  // Since 1.60 this is a _private_ function. You can
...
IMGUI_API ImVec2 FindBestWindowPosForPopupEx(
        ...
        const ImRect& r_avoid, ImGuiPopupPositionPolicy policy = ImGuiPopupPositionPolicy_Default);
...
IMGUI_API int CalcTypematicPressedRepeatAmount(float t, float t_prev, float repeat_delay,
                                               float repeat_rate);
...
IMGUI_API void TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags,
                                 ImU32 col);
IMGUI_API bool TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
                                          ImGuiTabItemFlags flags, const char* label,
...
IMGUI_API void RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col,
...
IMGUI_API void VerticalSeparator();  // Vertical separator, for menu bars (use current line height).
...
IMGUI_API bool ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held,
                              ImGuiButtonFlags flags = 0);
IMGUI_API bool DragBehavior(ImGuiID id, ImGuiDataType data_type, void* v, float v_speed,
                            const void* v_min, const void* v_max, const char* format, float power,
                            ImGuiDragFlags flags);
IMGUI_API bool SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* v,
                              const void* v_min, const void* v_max, const char* format, float power,
...
IMGUI_API bool TreeNodeBehaviorIsOpen(
        ImGuiID id, ImGuiTreeNodeFlags flags = 0);  // Consume previous SetNextTreeNodeOpened()
...
IMGUI_API bool DragBehaviorT(ImGuiDataType data_type, T* v, float v_speed, const T v_min,
                             const T v_max, const char* format, float power, ImGuiDragFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, T* v,
                               const T v_min, const T v_max, const char* format, float power,
                               ImGuiSliderFlags flags, ImRect* out_grab_bb);
template <typename T, typename FLOAT_T>
IMGUI_API float SliderCalcRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max,
...
IMGUI_API bool InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg,
                           ...
                           void* user_data = NULL);
IMGUI_API bool InputScalarAsWidgetReplacement(const ImRect& bb, ImGuiID id, const char* label,
...
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS) \
    ImGuiTestEngineHook_ItemInfo(&g, _ID, _LABEL, _FLAGS)  // Register status flags
#else
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS) \
    do {                                                 \
    } while (0)
