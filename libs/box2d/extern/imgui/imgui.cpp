...
- FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
  ...
  - How can I have multiple widgets with the same label or with an empty label? A primer on labels
and the ID Stack.
...
/*
 ...
 integrating imgui types with your own maths types.
 ...
 use imgui everywhere) ImGui::NewFrame();

        // Most of your application code here
        ImGui::Text("Hello, world!");
        MyGameUpdate(); // may use any ImGui functions, e.g. ImGui::Begin("My window");
 ImGui::Text("Hello, world!"); ImGui::End(); MyGameRender(); // may use any ImGui functions as well!

        // Render imgui, swap buffers
        // (You want to try calling EndFrame/Render as late as you can, to be able to use imgui in
 your own game rendering code) ImGui::EndFrame(); ImGui::Render(); ImDrawData* draw_data =
 ...
 ImGui const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.Data;   // index buffer generated by ImGui
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // The vast majority of draw calls will use the imgui texture atlas, which value
 you have set yourself during initialization. MyEngineBindTexture((MyTexture*)pcmd->TextureId);
 ...
 cases you need to pass on the inputs to imgui. Refer to the FAQ for more information.
 ...
 (Win32, Glfw, SDL2, etc.) from renderer (DX11, OpenGL, Vulkan,  etc.). old binding will still work
 as is, however prefer using the separated bindings as they will be updated to be multi-viewport
 conformant. when adopting new bindings follow the main.cpp code of your preferred examples/ folder
 to know which functions to call.
 ...
 using the more explicit ImGuiOnceUponAFrame.
 ...
 (will obsolete).
 - 2017/08/11 (1.51) - renamed ImGuiSetCond_*** types and flags to ImGuiCond_***. Kept redirection
 enums (will obsolete).
 ...
 by casting your value to ImTexture / void*, and vice-versa. Because both end-points (user code and
 rendering function) are under your control, you know exactly what is stored inside the ImTexture /
 void*. Examples:
 ...
 implicit type conversions. This way you'll be able to use your own types everywhere, e.g. passsing
 ...
 GetWindowDrawList() and draw to it in any way you like.
    - You can call ImGui::GetOverlayDrawList() and use this draw list to display contents over every
 other imgui windows.
...
#pragma clang diagnostic ignored \
        "-Wunknown-pragmas"  // warning : unknown warning group '-Wformat-pedantic *'        // not
                             // all warnings are known by all clang versions.. so ignoring warnings
...
#pragma clang diagnostic ignored \
        "-Wexit-time-destructors"  // warning : declaration requires an exit-time destructor //
                                   // exit-time destruction order is undefined. if MemFree() leads
                                   // to users code that has been disabled before exit it might
                                   // cause problems. ImGui coding style welcomes static/globals.
#pragma clang diagnostic ignored \
...
static void SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry,
                                           const char* line);
...
// NULL. CreateContext() will automatically set this pointer if it is NULL. Change to a different
// context by calling ImGui::SetCurrentContext(). If you use DLL hotreloading you might need to call
// SetCurrentContext() after reloading code from this file. ImGui functions are not thread-safe
// because of this pointer. If you want thread-safety to allow N threads to access N different
// contexts, you can:
// - Change this variable to use thread local storage. You may #define GImGui in imconfig.h for that
// purpose. Future development aim to make this context pointer explicit to all calls. Also read
...
ImGuiStyle::ImGuiStyle() {
    ...
    GrabRounding = 0.0f;       // Radius of grabs corners rounding. Set to 0.0f to have rectangular
                               // slider grabs.
    ...
    DisplayWindowPadding =
            ImVec2(19, 19);  // Window position are clamped to be visible within the display area by
                             // at least this amount. Only applies to regular windows.
    DisplaySafeAreaPadding =
            ImVec2(3, 3);  // If you cannot see the edge of your screen (e.g. on a TV) increase the
    ...
    AntiAliasedLines = true;  // Enable anti-aliasing on lines/borders. Disable if you are really
                              // short on CPU/GPU.
    AntiAliasedFill =
...
// Pay attention that data_size==0 will yield different results than passing strlen(data) because
// the zero-terminated codepath handles ###. This should technically be split into two distinct
// functions (ImHashData/ImHashStr), perhaps once we remove the silly static variable.
...
ImU32 ImGui::GetColorU32(ImU32 col) {
    ...
    a = (ImU32)(a * style_alpha);  // We don't need to clamp 0..255 because Style.Alpha is in 0..1
                                   // range.
...
void ImGuiTextBuffer::appendfv(const char* fmt, va_list args) {
    ...
    int len = ImFormatStringV(NULL, 0, fmt, args);  // FIXME-OPT: could do a first pass write
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
    ...
    window->DC.CursorPosPrevLine.y =
            ...
            line_height;  // Setting those fields so that SetScrollHereY() can properly function
                          // after the end of our clipper usage.
    window->DC.PrevLineSize.y =
            (line_height -
             GImGui->Style.ItemSpacing
                     .y);  // If we end up needing more accurate data (to e.g. use SameLine) we may
                           // as well make the clipper have a fourth step to let user process and
                           // display the last item in their list.
...
void ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale) {
    ...
    g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b, center + c,
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg) {
    ...
    if (id != 0) {
        ...
        //  to regular navigation requests unfortunately, but it is still limited to one window.
        //      it may not scale very well for windows with ten of thousands of item, but at least
        //      NavMoveRequest is only set on user interaction, aka maximum once a frame. We could
        //      early out with "if (is_clipped && !g.NavInitRequest) return false;" but when we
        //      wouldn't be able to reach unclipped widgets. This would work if user had explicit
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags) {
    ...
    // Special handling for the 1st item after Begin() which represent the title bar. When the
    // window is collapsed (SkipItems==true) that last item will never be overwritten so we need to
    // detect tht case.
...
// unit and imgui.cpp's compilation unit If the user has inconsistent compilation settings, imgui
// configuration #define, packing pragma, etc. you may see different structures from what imgui.cpp
// sees which is highly problematic.
...
ImGuiIO& ImGui::GetIO() {
    IM_ASSERT(GImGui != NULL && "No current context. Did you call ImGui::CreateContext() or "
...
void ImGui::StartMouseMovingWindow(ImGuiWindow* window) {
    ...
    // afterward. This is because we want ActiveId to be set even when the window is stuck from
    // moving.
    ImGuiContext& g = *GImGui;
    FocusWindow(window);
...
void ImGui::UpdateMouseMovingWindowEndFrame() {
    ...
    // (The left mouse button path calls FocusWindow which will lead NewFrame->ClosePopupsOverWindow
    // to trigger)
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
        ...
        if (g.IO.MouseClicked[i]) {
            ...
            g.IO.MouseDragMaxDistanceAbs[i].x =
                    ImMax(g.IO.MouseDragMaxDistanceAbs[i].x, delta_from_click_pos.x < 0.0f ?
                                                                     -delta_from_click_pos.x :
                                                                     delta_from_click_pos.x);
            g.IO.MouseDragMaxDistanceAbs[i].y =
                    ImMax(g.IO.MouseDragMaxDistanceAbs[i].y, delta_from_click_pos.y < 0.0f ?
...
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
    ...
    // false = dispatch mouse info to imgui + app)
    ...
    // imgui, false = dispatch keyboard info to imgui + app)
...
static void AddWindowToSortBuffer(ImVector<ImGuiWindow*>* out_sorted_windows, ImGuiWindow* window) {
    ...
    if (window->Active) {
        ...
        if (count > 1)
            ImQsort(window->DC.ChildWindows.begin(), (size_t)count, sizeof(ImGuiWindow*),
                    ChildWindowComparer);
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list) {
    ...
    // you are drawing lots of stuff manually: A) Make sure you are coarse clipping, because
    // ImDrawList let all your vertices pass. You can use the Metrics window to inspect draw list
    // contents. B) If you need/want meshes with more than 64K vertices, uncomment the '#define
    // ImDrawIdx unsigned int' line in imconfig.h to set the index size to 4 bytes.
    //    You'll need to handle the 4-bytes indices to your renderer. For example, the OpenGL
    //    example code detect index size at compile-time by doing:
    ...
    //    index sizes. 2 and 4 bytes indices are generally supported by most API.
    // C) If for some reason you cannot use 4 bytes indices or don't want to, a workaround is to
    // call BeginChild()/EndChild() before reaching the 64K limit to split your draw commands in
    // multiple draw lists.
...
void ImGui::Render() {
    ...
    for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++)
        if (windows_to_render_front_most[n] &&
            IsWindowActiveAndVisible(
                    windows_to_render_front_most[n]))  // NavWindowingTarget is always temporarily
    ...
    if (g.IO.MouseDrawCursor)
        RenderMouseCursor(&g.OverlayDrawList, g.IO.MousePos, g.Style.MouseCursorScale,
                          g.MouseCursor);
...
int ImGui::CalcTypematicPressedRepeatAmount(float t, float t_prev, float repeat_delay,
                                            float repeat_rate) {
    if (t == 0.0f)
        return 1;
...
// NB: This is only valid if IsMousePosValid(). Back-ends in theory should always keep mouse
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
    ...
    if (g.IO.MouseDown[button])
        if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
            return g.IO.MousePos -
                   g.IO.MouseClickedPos[button];  // Assume we can only get active with left-mouse
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags) {
    ...
    window->DC.CursorMaxPos =
            window->Pos;  // So first call to CalcSizeContents() doesn't return crazy values
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window, const ImVec2& size_contents) {
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip) {
        ...
        // expected scrollbar. FIXME: Might turn bigger than DisplaySize-WindowPadding.
...
static void ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit,
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
        ...
        // GetOverlayDrawList(window)->AddRect(resize_rect.Min, resize_rect.Max, IM_COL32(255, 255,
        // 0, 255));
        if (hovered || held)
            g.MouseCursor =
        ...
        if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
            ...
            ImVec2 corner_target = g.IO.MousePos - g.ActiveIdClickOffset +
                                   ImLerp(grip.InnerDir * grip_hover_outer_size,
                                          ...
                                          grip.CornerPos);  // Corner of the window corresponding to
                                                            // our corner grip
            CalcResizePosSizeFromAnyCorner(window, corner_target, grip.CornerPos, &pos_target,
                                           &size_target);
        }
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++) {
        ...
        // GetOverlayDrawList(window)->AddRect(border_rect.Min, border_rect.Max, IM_COL32(255, 255,
        // 0, 255));
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
    ...
    if (window_just_created) {
        ImVec2 size_on_first_use =
                (g.NextWindowData.SizeCond != 0) ?
                        ...
                        ImVec2(0.0f, 0.0f);  // Any condition flag will do since we are creating a
                                             // new window here.
    ...
    if (flags & ImGuiWindowFlags_Popup) {
        ...
        window_just_activated_by_user |=
                (window->PopupId != popup_ref.PopupId);  // We recycle popups so treat window as
    ...
    if (first_begin_of_the_frame) {
        ...
        // windows) We reset Size/SizeContents for reappearing popups/tooltips early in this
        ...
        // inconsistencies)
        window->WindowBorderSize = (flags & ImGuiWindowFlags_ChildWindow) ? style.ChildBorderSize :
        ...
        if ((flags & ImGuiWindowFlags_AlwaysAutoResize) && !window->Collapsed) {
            ...
            if (!window_size_x_set_by_api && window->AutoFitFramesX > 0)
                window->SizeFull.x = size_full_modified.x =
                        window->AutoFitOnlyGrows ? ImMax(window->SizeFull.x, size_auto_fit.x) :
                                                   size_auto_fit.x;
            if (!window_size_y_set_by_api && window->AutoFitFramesY > 0)
                window->SizeFull.y = size_full_modified.y =
                        window->AutoFitOnlyGrows ? ImMax(window->SizeFull.y, size_auto_fit.y) :
                                                   size_auto_fit.y;
            if (!window->Collapsed)
                MarkIniSettingsDirty(window);
        }
        ...
        if (!window->Collapsed) {
            ...
            if (window->ScrollbarX && !window->ScrollbarY)
                window->ScrollbarY =
                        (window->SizeContents.y > size_y_for_scrollbars - style.ScrollbarSize) &&
                        !(flags & ImGuiWindowFlags_NoScrollbar);
        ...
        const bool window_pos_with_pivot =
                (window->SetWindowPosVal.x != FLT_MAX && window->HiddenFramesForResize == 0);
        ...
        if (dim_bg_for_window_list && window == g.NavWindowingTargetAnim) {
            ...
            if (!bb.Contains(viewport_rect))  // Avoid drawing if the window covers all the viewport
                                              // anyway
        ...
        if (window->Collapsed) {
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
                ImU32 title_bar_col = GetColorU32(window->Collapsed ? ImGuiCol_TitleBgCollapsed :
            ...
            if (flags & ImGuiWindowFlags_MenuBar) {
                ...
                menu_bar_rect.ClipWith(window->Rect());  // Soft clipping, in particular child
        ...
        // FIXME: window->ContentsRegionRect.Max is currently very misleading / partly faulty, but
        // some BeginChild() patterns relies on it.
        window->ContentsRegionRect.Min.x =
                window->Pos.x - window->Scroll.x + window->WindowPadding.x;
        window->ContentsRegionRect.Min.y = window->Pos.y - window->Scroll.y +
                                           window->WindowPadding.y + window->TitleBarHeight() +
                                           window->MenuBarHeight();
        window->ContentsRegionRect.Max.x = window->Pos.x - window->Scroll.x -
                                           window->WindowPadding.x +
                                           (window->SizeContentsExplicit.x != 0.0f ?
                                                    window->SizeContentsExplicit.x :
                                                    (window->Size.x - window->ScrollbarSizes.x));
        window->ContentsRegionRect.Max.y = window->Pos.y - window->Scroll.y -
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
            ...
            if (!(flags & ImGuiWindowFlags_NoCollapse))
                if (CollapseButton(window->GetID("#COLLAPSE"), window->Pos))
                    window->WantCollapseToggle = true;  // Defer collapsing to next frame as we are
            ...
            if (p_open != NULL) {
                ...
                if (CloseButton(window->GetID("#CLOSE"),
                                window->Rect().GetTR() + ImVec2(-pad - rad, pad + rad), rad + 1))
                    *p_open = false;
            }
            ...
            // Title bar text (with: horizontal alignment, avoiding collapse/close button, optional
            // "unsaved document" marker)
            // FIXME: Refactor text alignment facilities along with RenderText helpers, this is too
            // much code..
            const char* UNSAVED_DOCUMENT_MARKER = "*";
            float marker_size_x = (flags & ImGuiWindowFlags_UnsavedDocument) ?
            ...
            if (flags & ImGuiWindowFlags_UnsavedDocument) {
                ImVec2 marker_pos = ImVec2(ImMax(text_r.Min.x,
                ...
                RenderTextClipped(marker_pos + off, text_r.Max + off, UNSAVED_DOCUMENT_MARKER, NULL,
                                  NULL, ImVec2(0, style.WindowTitleAlign.y), &clip_rect);
            }
        }
        ...
        */
        ...
        // Inner rectangle
        // We set this up after processing the resize grip so that our clip rectangle doesn't lag by
        // a frame Note that if our window is collapsed we will end up with an inverted (~null)
        // clipping rectangle which is the correct behavior.
        ...
        window->InnerClipRect.Min.x = ImFloor(
                0.5f + window->InnerMainRect.Min.x +
                ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - window->WindowBorderSize)));
        window->InnerClipRect.Min.y = ImFloor(0.5f + window->InnerMainRect.Min.y);
        window->InnerClipRect.Max.x = ImFloor(
                0.5f + window->InnerMainRect.Max.x -
                ImMax(0.0f, ImFloor(window->WindowPadding.x * 0.5f - window->WindowBorderSize)));
        window->InnerClipRect.Max.y = ImFloor(0.5f + window->InnerMainRect.Max.y);

        // After Begin() we fill the last item / hovered data based on title bar data. It is a
        // standard behavior (to allow creation of context menus on title bar only, etc.).
...
void ImGui::PushMultiItemsWidths(int components, float w_full) {
    ...
    const float w_item_last = ImMax(
            1.0f,
            (float)(int)(w_full - (w_item_one + style.ItemInnerSpacing.x) * (components - 1)));
...
void ImGui::SetWindowScrollX(ImGuiWindow* window, float new_scroll_x) {
    window->DC.CursorMaxPos.x +=
            window->Scroll.x;  // SizeContents is generally computed based on CursorMaxPos which is
...
void ImGui::SetWindowScrollY(ImGuiWindow* window, float new_scroll_y) {
    window->DC.CursorMaxPos.y +=
            window->Scroll.y;  // SizeContents is generally computed based on CursorMaxPos which is
                               // affected by scroll position, so we need to apply our change to it.
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond) {
    ...
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond));  // Make sure the user doesn't attempt to combine
    ...
    window->DC.CursorPos +=
            (window->Pos -
             old_pos);  // As we happen to move the window while it is being appended to (which is a
                        // bad idea - will smear) let's at least offset the cursor
    window->DC.CursorMaxPos +=
            (window->Pos - old_pos);  // And more importantly we need to adjust this so size
...
void ImGui::SetNextWindowContentSize(const ImVec2& size) {
    ...
    g.NextWindowData.ContentSizeVal =
            size;  // In Begin() we will add the size of window decorations (title bar, menu etc.)
                   // to that to form a SizeContents value.
...
void ImGui::SetNextWindowFocus() {
    ...
    g.NextWindowData.FocusCond =
            ImGuiCond_Always;  // Using a Cond member for consistency (may transition all of them to
                               // single flag set for fast Clear() op)
}
...
float ImGui::GetScrollMaxX() { return GetWindowScrollMaxX(GImGui->CurrentWindow); }

float ImGui::GetScrollMaxY() { return GetWindowScrollMaxY(GImGui->CurrentWindow); }
...
void ImGui::EndGroup() {
    ...
    if (group_data.AdvanceCursor) {
        window->DC.CurrentLineTextBaseOffset =
                ImMax(window->DC.PrevLineTextBaseOffset,
                      group_data.BackupCurrentLineTextBaseOffset);  // FIXME: Incorrect, we should
    ...
    // LastItemIsActive', but would put a little more burden on individual widgets. (and if you grep
    // for LastItemId you'll notice it is only used in that context.
    if ((group_data.BackupActiveIdIsAlive != g.ActiveId) && (g.ActiveIdIsAlive == g.ActiveId) &&
        ...
        window->DC.LastItemId = g.ActiveId;
             ...
             g.ActiveIdPreviousFrameIsAlive)  // && g.ActiveIdPreviousFrameWindow->RootWindow ==
                                              // window->RootWindow)
...
void ImGui::OpenPopupEx(ImGuiID id) {
    ...
    ImGuiPopupRef popup_ref;  // Tagged as new ref as Window will be set back to NULL if we write
...
bool ImGui::OpenPopupOnItemClick(const char* str_id, int mouse_button) {
    ...
    if (IsMouseReleased(mouse_button) && IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup)) {
        ImGuiID id = str_id ? window->GetID(str_id) :
                              window->DC.LastItemId;  // If user hasn't passed an ID, we can use the
                                                      // LastItemID. Using LastItemID as a Popup ID
                                                      // won't conflict!
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window) {
    ...
    if (ref_window) {
        ...
        for (; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++) {
            ...
            // Trim the stack if popups are not direct descendant of the reference window (which is
            // often the NavWindow)
            bool popup_or_descendent_has_focus = false;
            for (int m = popup_count_to_keep;
                 m < g.OpenPopupStack.Size && !popup_or_descendent_has_focus; m++)
...
void ImGui::ClosePopupToLevel(int remaining, bool apply_focus_to_window_under) {
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* focus_window = (remaining > 0) ? g.OpenPopupStack[remaining - 1].Window :
                                                  g.OpenPopupStack[0].ParentWindow;
    g.OpenPopupStack.resize(remaining);

    // FIXME: This code is faulty and we may want to eventually to replace or remove the
    // 'apply_focus_to_window_under=true' path completely. Instead of using
    // g.OpenPopupStack[remaining-1].Window etc. we should find the highest root window that is
    // behind the popups we are closing. The current code will set focus to the parent of the popup
    // window which is incorrect. It rarely manifested until now because UpdateMouseMovingWindow()
    // would call FocusWindow() again on the clicked window, leading to a chain of focusing A
    // (clicked window) then B (parent window of the popup) then A again. However if the clicked
    // window has the _NoMove flag set we would be left with B focused. For now, we have disabled
    // this path when called from ClosePopupsOverWindow() because the users of
    // ClosePopupsOverWindow() don't need to alter focus anyway, but we should inspect and fix this
    // properly.
...
// which will close the popup. Note that popup visibility status is owned by imgui (and manipulated
// with e.g. OpenPopup) so the actual value of *p_open is meaningless here.
...
bool ImGui::BeginPopupContextItem(const char* str_id, int mouse_button) {
    ...
    ImGuiID id =
            str_id ?
                    window->GetID(str_id) :
                    window->DC
                            .LastItemId;  // If user hasn't passed an ID, we can use the LastItemID.
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window) {
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu) {
        ...
        // FindBestWindowPosForPopup() function will move the new menu outside the parent bounds.
        // This is how we end up with child menus appearing (most-commonly) on the right of the
        // parent menu.
        IM_ASSERT(g.CurrentWindow == window);
        ImGuiWindow* parent_window = g.CurrentWindowStack[g.CurrentWindowStack.Size - 2];
        float horizontal_overlap =
                g.Style.ItemSpacing.x;  // We want some overlap to convey the relative depth of each
                                        // menu (currently the amount of overlap is hard-coded to
                                        // style.ItemSpacing.x).
        ImRect r_avoid;
        if (parent_window->DC.MenuBarAppending)
            r_avoid = ImRect(-FLT_MAX, parent_window->Pos.y + parent_window->TitleBarHeight(),
                             FLT_MAX,
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip) {
        ...
        if (window->AutoPosLastDirection == ImGuiDir_None)
            pos = ref_pos + ImVec2(2, 2);  // If there's not enough room, for tooltip we prefer
                                           // avoiding the cursor at all cost even if it means that
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand) {
    ...
    const ImRect& curr = g.NavScoringRectScreen;  // Current modified source rect (NB: we've applied
    ...
    float dby = NavScoreItemDistInterval(
            ...
            ImLerp(curr.Min.y, curr.Max.y, 0.8f));  // Scale down on Y to keep using box-distance
                                                    // for vertically touching items
...
static void NavScrollToBringItemIntoView(ImGuiWindow* window, const ImRect& item_rect) {
    ...
    // GetOverlayDrawList(window)->AddRect(window_rect.Min, window_rect.Max, IM_COL32_WHITE); //
    // [DEBUG]
...
#undef NAV_MAP_KEY
    ...
    IM_ASSERT(
            !g.NavScoringRectScreen.IsInverted());  // Ensure if we have a finite, non-inverted
                                                    // bounding box here will allows us to remove
                                                    // extraneous ImFabs() calls in NavScoreItem().
    // g.OverlayDrawList.AddRect(g.NavScoringRectScreen.Min, g.NavScoringRectScreen.Max,
    ...
    if (g.NavMoveDir == ImGuiDir_None && g.NavWindow &&
        ...
        if ((page_up_held && !page_down_held) || (page_down_held && !page_up_held)) {
            if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
                ...
                const float page_offset_y = ImMax(0.0f, window->InnerClipRect.GetHeight() -
...
static void ImGui::NavUpdateWindowing() {
    ...
    if (start_windowing_with_gamepad || start_windowing_with_keyboard)
        if (ImGuiWindow* window = g.NavWindow ? g.NavWindow :
    ...
    if (g.NavWindowingTarget && g.NavInputSource == ImGuiInputSource_NavGamepad) {
        ...
        // then the window was merely rendered front-most)
    ...
    if (apply_toggle_layer && g.NavWindow) {
        ...
        g.NavDisableHighlight = false;
        g.NavDisableMouseHover = true;
        NavRestoreLayer((g.NavWindow->DC.NavLayerActiveMask & (1 << ImGuiNavLayer_Menu)) ?
...
void ImGui::SetColumnOffset(int column_index, float offset) {
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
        offset = ImMin(offset,
                       columns->MaxX - g.Style.ColumnsMinSpacing * (columns->Count - column_index));
...
void ImGui::EndColumns() {
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_GrowParentContentsSize))
        window->DC.CursorMaxPos.x =
                columns->StartMaxPosX;  // Restore cursor max pos, as columns don't grow parent
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems) {
        ...
        for (int n = 1; n < columns->Count; n++) {
            ...
            // Draw column (we clip the Y boundaries CPU side because very long triangles are
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags) {
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern)) {
        ...
        if (source_id == 0) {
            ...
            bool is_hovered =
                    (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) != 0;
...
static void* SettingsHandlerWindow_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*,
                                            const char* name) {
...
void ImGui::ShowMetricsWindow(bool* p_open) {
    ...
    struct Funcs {
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label) {
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin();
                ...
                ImGuiListClipper clipper(
                        pcmd->ElemCount /
                        3);  // Manually coarse clip our print out of individual vertices to save
                             // CPU, only items that may be visible.
                while (clipper.Step())
                        ...
                        if (ImGui::IsItemHovered()) {
                            ...
                            overlay_draw_list->Flags &=
                                    ~ImDrawListFlags_AntiAliasedLines;  // Disable AA on triangle
                                                                        ...
                                                                        // and thin triangles.
                            overlay_draw_list->AddPolyline(triangles_pos, 3,
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label) {
            ...
            ImGui::BulletText("Pos: (%.1f,%.1f), Size: (%.1f,%.1f), SizeContents (%.1f,%.1f)",
            ...
            ImGui::BulletText("Appearing: %d, Hidden: %d (Reg %d Resize %d), SkipItems: %d",
    ...
    if (g.IO.KeyCtrl && show_window_begin_order) {
        for (int n = 0; n < g.Windows.Size; n++) {
            ...
            overlay_draw_list->AddText(NULL, font_size, window->Pos, IM_COL32(255, 255, 255, 255),
                                       buf);
        }
    }
    ImGui::End();
