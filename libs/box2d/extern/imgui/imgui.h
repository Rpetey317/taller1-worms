...
ImColor)
// Draw List API (ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel, ImDrawListFlags, ImDrawList,
ImDrawData)
...
// typically starts at XYY00 then bounced up to XYY01 when release tagging happens)
...
struct ImDrawChannel;  // Temporary storage for ImDrawList ot output draw commands out of order,
                       // used by ImDrawList::ChannelsSplit()
...
IMGUI_API bool DebugCheckVersionAndDataLayout(const char* version_str, size_t sz_io,
...
IMGUI_API void NewFrame();  // start a new ImGui frame, you can submit any command from this point
                            // until Render()/EndFrame().
IMGUI_API void EndFrame();  // ends the ImGui frame. automatically called by Render(), you likely
                            // don't need to call that yourself directly. If you don't need to
                            // render data (skipping rendering) you may call EndFrame() but you'll
                            // have wasted CPU already! If you don't need to render, better to not
                            // create any imgui windows and not call NewFrame() at all!
IMGUI_API void Render();    // ends the ImGui frame, finalize the draw data. (Obsolete: optionally
                          // call io.RenderDrawListsFn if set. Nowadays, prefer calling your render
                          // function yourself.)
...
IMGUI_API void ShowDemoWindow(
        bool* p_open = NULL);  // create demo/test window (previously called ShowTestWindow).
...
IMGUI_API void ShowMetricsWindow(
        bool* p_open = NULL);  // create metrics window. display Dear ImGui internals: draw commands
                               // (with individual draw calls and vertices), window list, basic
                               // internal state, etc.
...
IMGUI_API void ShowFontSelector(
        const char* label);  // add font selector block (not a window), essentially a combo listing
                             // the loaded fonts.
...
IMGUI_API ImDrawList* GetWindowDrawList();  // get draw list associated to the window, to append
...
IMGUI_API ImVec2 GetWindowContentRegionMin();  // content boundaries min (roughly (0,0)-Scroll), in
                                               // window coordinates
IMGUI_API ImVec2 GetWindowContentRegionMax();  // content boundaries max (roughly (0,0)+Size-Scroll)
                                               // where Size can be override with
...
IMGUI_API void SetNextWindowSize(
        const ImVec2& size, ImGuiCond cond = 0);  // set next window size. set axis to 0.0f to force
...
IMGUI_API void SetNextWindowContentSize(
        const ImVec2& size);  // set next window content size (~ enforce the range of scrollbars).
                              // not including window decorations (title bar, menu bar, etc.). set
...
IMGUI_API void SetWindowSize(
        const ImVec2& size,
        ImGuiCond cond =
                0);  // (not recommended) set current window size - call within Begin()/End(). set
                     // to ImVec2(0,0) to force an auto-fit. prefer using SetNextWindowSize(), as
                     // this may incur tearing and minor side-effects.
IMGUI_API void SetWindowCollapsed(
        bool collapsed, ImGuiCond cond = 0);  // (not recommended) set current window collapsed
...
IMGUI_API void SetScrollFromPosY(
        ...
        float center_y_ratio =
                0.5f);  // adjust scrolling amount to make given position valid. use GetCursorPos()
...
IMGUI_API const ImVec4& GetStyleColorVec4(
        ImGuiCol idx);  // retrieve style color as stored in ImGuiStyle structure. use to feed back
                        // into PushStyleColor(), otherwise use GetColorU32() to get style color
                        // with style alpha baked in.
...
IMGUI_API void PushItemWidth(
        float item_width);  // width of items for the common item+label case, pixels. 0.0f = default
...
IMGUI_API void PushTextWrapPos(
        float wrap_local_pos_x = 0.0f);  // word-wrapping for Text*() commands. < 0.0f: no wrapping;
...
IMGUI_API void PushAllowKeyboardFocus(
        bool allow_keyboard_focus);  // allow focusing using TAB/Shift-TAB, enabled by default but
...
IMGUI_API void PushButtonRepeat(
        bool repeat);  // in 'repeat' mode, Button*() functions return repeated true in a typematic
                       // manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you
                       // can call IsItemActive() after any Button() to tell if the button is held
...
IMGUI_API void SameLine(
        ...
        float spacing_w = -1.0f);  // call between widgets or groups to layout them horizontally. X
...
IMGUI_API void Dummy(
        const ImVec2& size);  // add a dummy item of given size. unlike InvisibleButton(), Dummy()
...
IMGUI_API float GetCursorPosY();  //    other functions such as GetCursorScreenPos or everything in
                                  //    ImDrawList::
...
IMGUI_API void PushID(const char* str_id);  // push string identifier into the ID stack. IDs == hash
                                            // of the entire stack!
...
IMGUI_API ImGuiID GetID(
        const char* str_id);  // calculate unique ID (hash of whole ID stack + given parameter).
...
IMGUI_API void TextUnformatted(
        const char* text,
        const char* text_end =
                NULL);  // raw text without formatting. Roughly equivalent to Text("%s", text) but:
...
IMGUI_API bool InvisibleButton(
        const char* str_id,
        const ImVec2&
                size);  // button behavior without the visuals, useful to build custom behaviors
...
IMGUI_API void
        Bullet();  // draw a small circle and keep the cursor on the same line. advance cursor x
...
IMGUI_API bool DragScalar(const char* label, ImGuiDataType data_type, void* v, float v_speed,
                          const void* v_min = NULL, const void* v_max = NULL,
                          const char* format = NULL, float power = 1.0f);
IMGUI_API bool DragScalarN(const char* label, ImGuiDataType data_type, void* v, int components,
                           float v_speed, const void* v_min = NULL, const void* v_max = NULL,
                           const char* format = NULL, float power = 1.0f);
...
IMGUI_API bool SliderFloat(
        ...
        float power =
                1.0f);  // adjust format to decorate the value with a prefix or a suffix for
...
IMGUI_API bool SliderInt4(const char* label, int v[4], int v_min, int v_max,
                          const char* format = "%d");
IMGUI_API bool SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min,
                            const void* v_max, const char* format = NULL, float power = 1.0f);
IMGUI_API bool SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components,
                             const void* v_min, const void* v_max, const char* format = NULL,
                             float power = 1.0f);
...
IMGUI_API bool InputScalar(const char* label, ImGuiDataType data_type, void* v,
                           const void* step = NULL, const void* step_fast = NULL,
...
// is just a way to document the number of elements that are expected to be accessible. You can the
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
        IM_FMTARGS(2);  // helper variation to completely decorelate the id from the displayed
                        // string. Read the FAQ about why and how to use ID. to align arbitrary text
                        // at the same level as a TreeNode() you can use Bullet().
...
IMGUI_API bool CollapsingHeader(
        ...
        ImGuiTreeNodeFlags flags =
                0);  // if returning 'true' the header is open. doesn't indent nor push on ID stack.
...
IMGUI_API bool ListBoxHeader(
        const char* label,
        const ImVec2& size =
                ImVec2(0, 0));  // use if you want to reimplement ListBox() will custom data or
...
IMGUI_API bool MenuItem(
        ...
        bool enabled = true);  // return true when activated. shortcuts are displayed for
...
IMGUI_API void OpenPopup(
        const char* str_id);  // call to mark popup as open (don't call every frame!). popups are
                              // closed when user click outside, or if CloseCurrentPopup() is called
                              // within a BeginPopup()/EndPopup() block. By default,
                              // Selectable()/MenuItem() are calling CloseCurrentPopup(). Popup
                              // identifiers are relative to the current ID-stack (so OpenPopup and
...
IMGUI_API bool BeginPopup(const char* str_id,
                          ImGuiWindowFlags flags = 0);  // return true if the popup is open, and you
                                                        // can start outputting to it. only call
                                                        // EndPopup() if BeginPopup() returns true!
IMGUI_API bool BeginPopupContextItem(
        const char* str_id = NULL,
        int mouse_button = 1);  // helper to open and begin popup when clicked on last item. if you
...
IMGUI_API bool BeginPopupContextVoid(
        const char* str_id = NULL,
        int mouse_button = 1);  // helper to open and begin popup when clicked in void (where there
                                // are no imgui windows).
IMGUI_API bool BeginPopupModal(
        const char* name, bool* p_open = NULL,
        ImGuiWindowFlags flags =
                0);  // modal dialog (regular window with title bar, block interactions behind the
                     // modal window, can't close the modal window by clicking outside)
...
IMGUI_API void
        CloseCurrentPopup();  // close the popup we have begin-ed into. clicking on a MenuItem or
...
IMGUI_API void NextColumn();  // next column, defaults to current row or next row if the current row
                              // is finished
...
IMGUI_API bool BeginDragDropSource(
        ImGuiDragDropFlags flags =
                0);  // call when the current item is active. If this return true, you can call
...
IMGUI_API void EndDragDropSource();    // only call EndDragDropSource() if BeginDragDropSource()
                                       // returns true!
IMGUI_API bool BeginDragDropTarget();  // call after submitting an item that may receive a payload.
                                       // If this returns true, you can call AcceptDragDropPayload()
                                       // + EndDragDropTarget()
...
IMGUI_API void EndDragDropTarget();  // only call EndDragDropTarget() if BeginDragDropTarget()
                                     // returns true!
...
IMGUI_API bool IsItemHovered(
        ImGuiHoveredFlags flags = 0);  // is the last item hovered? (and usable, aka not blocked by
...
IMGUI_API bool
        IsItemActive();  // is the last item active? (e.g. button being held, text field being
                         // edited. This will continuously return true while holding mouse button on
                         // an item. Items that don't interact will always return false)
...
IMGUI_API bool IsItemVisible();  // is the last item visible? (items may be out of sight because of
                                 // clipping/scrolling)
IMGUI_API bool IsItemEdited();   // did the last item modify its underlying value this frame? or was
                                // pressed? This is generally the same as the "bool" return value of
                                // many widgets.
...
IMGUI_API bool
        IsItemDeactivatedAfterEdit();  // was the last item just made inactive and made a value
                                       ...
                                       // Useful for Undo/Redo patterns with widgets that requires
...
IMGUI_API ImDrawList* GetOverlayDrawList();  // this draw list will be the last rendered one, useful
...
IMGUI_API bool IsMouseDragging(
        int button = 0, float lock_threshold = -1.0f);  // is mouse dragging. if lock_threshold <
...
IMGUI_API ImVec2
        GetMousePosOnOpeningCurrentPopup();  // retrieve backup of mouse position at the time of
                                             // opening popup we have BeginPopup() into
IMGUI_API ImVec2 GetMouseDragDelta(
        int button = 0,
        float lock_threshold =
                -1.0f);  // return the delta from the initial clicking position. This is locked and
                         // return 0.0f until the mouse moves past a distance threshold at least
                         // once. If lock_threshold < -1.0f uses io.MouseDraggingThreshold
IMGUI_API void ResetMouseDragDelta(int button = 0);  //
IMGUI_API ImGuiMouseCursor
        GetMouseCursor();  // get desired cursor type, reset in ImGui::NewFrame(), this is updated
...
IMGUI_API void CaptureKeyboardFromApp(
        bool want_capture_keyboard_value =
                true);  // attention: misleading name! manually override io.WantCaptureKeyboard flag
                        // next frame (said flag is entirely left for your application to handle).
                        // e.g. force capture keyboard when your widget is being hovered. This is
...
IMGUI_API void CaptureMouseFromApp(
        bool want_capture_mouse_value =
                true);  // attention: misleading name! manually override io.WantCaptureMouse flag
                        // next frame (said flag is entirely left for your application to handle).
                        // This is equivalent to setting "io.WantCaptureMouse =
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_UnsavedDocument =
            1 << 20,  // Append '*' to title without affecting the ID, as a convenience to avoid
                      // using the ### operator. When used in a tab/docking context, tab is selected
                      // on closure and closure is deferred by one frame to allow code to cancel the
    ...
    ImGuiWindowFlags_NavFlattened =
            1 << 23,  // [BETA] Allow gamepad/keyboard navigation to cross over parent border to
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_EnterReturnsTrue = 1 << 5,  // Return 'true' when Enter is pressed (as
    ...
    ImGuiInputTextFlags_CallbackCharFilter =
            1 << 9,  // Callback on character inputs to replace or discard them. Modify 'EventChar'
    ...
    ImGuiInputTextFlags_NoUndoRedo =
            1 << 16,  // Disable undo/redo. Note that input text owns the text data while active, if
...
enum ImGuiComboFlags_ {
    ...
    ImGuiComboFlags_HeightSmall =
            1
            << 1,  // Max ~4 items visible. Tip: If you want your combo popup to be a specific size
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_UnsavedDocument =
            1 << 0,  // Append '*' to title without affecting the ID, as a convenience to avoid
                     // using the ### operator. Also: tab is selected on closure and closure is
                     // deferred by one frame to allow code to undo it without flicker.
    ImGuiTabItemFlags_SetSelected = 1 << 1,  // Trigger flag to programatically make the tab
...
enum ImGuiFocusedFlags_ {
    ...
    ImGuiFocusedFlags_AnyWindow =
            1 << 2,  // IsWindowFocused(): Return true if any window is focused. Important: If you
                     // are trying to tell how to dispatch your low-level inputs, do NOT use this.
                     // Use ImGui::GetIO().WantCaptureMouse instead.
...
enum ImGuiHoveredFlags_ {
    ImGuiHoveredFlags_None =
            0,  // Return true if directly over the item/window, not obstructed by another window,
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByPopup =
            1 << 3,  // Return true even if a popup window is normally blocking access to this
                     // item/window
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceNoPreviewTooltip =
            1 << 0,  // By default, a successful call to BeginDragDropSource opens a tooltip so you
    ...
    ImGuiDragDropFlags_SourceNoDisableHover =
            1 << 1,  // By default, when dragging we clear data so that IsItemHovered() will return
                     // false, to avoid subsequent user code submitting tooltips. This flag disable
    ...
    ImGuiDragDropFlags_SourceExtern =
            1 << 4,  // External source (from outside of imgui), won't attempt to read current
    ...
    ImGuiDragDropFlags_AcceptNoPreviewTooltip =
            1 << 12,  // Request hiding the BeginDragDropSource tooltip from the BeginDragDropTarget
                      // site.
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableSetMousePos =
            1 << 2,  // Instruct navigation to move the mouse cursor. May be useful on TV/console
                     // systems where moving a virtual mouse is awkward. Will update io.MousePos and
                     // set io.WantSetMousePos=true. If enabled you MUST honor io.WantSetMousePos
                     // requests in your binding, otherwise ImGui will react as if the mouse is
    ...
    // multiple projects. Those flags are not used by core ImGui)
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseCursors = 1 << 1,  // Back-end supports honoring GetMouseCursor()
                                                 // value to change the OS cursor shape.
    ImGuiBackendFlags_HasSetMousePos =
            1 << 2  // Back-end supports io.WantSetMousePos requests to reposition the OS mouse
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoOptions =
            1 << 3,  //              // ColorEdit: disable toggling options menu when right-clicking
                     //              on inputs/small preview.
    ImGuiColorEditFlags_NoSmallPreview =
            1 << 4,  //              // ColorEdit, ColorPicker: disable colored square preview next
    ...
    ImGuiColorEditFlags_NoSidePreview =
            1 << 8,  //              // ColorPicker: disable bigger color preview on right side of
                     //              the picker, use small colored square preview instead.
    ImGuiColorEditFlags_NoDragDrop =
            1 << 9,  //              // ColorEdit: disable drag and drop target. ColorButton:
                     //              disable drag and drop source.

    // User Options (right-click on widget to change some of them). You can set application defaults
    // using SetColorEditOptions(). The idea is that you probably don't want to override them in
    // most of your calls, let the user choose and/or call SetColorEditOptions() during startup.
    ...
    ImGuiColorEditFlags_HDR =
            1 << 19,  //              // (WIP) ColorEdit: Currently only disable 0.0f..1.0f limits
                      //              in RGBA edition (note: you probably want to use
                      //              ImGuiColorEditFlags_Float flag as well).
    ImGuiColorEditFlags_RGB = 1 << 20,  // [Inputs]     // ColorEdit: choose one among RGB/HSV/HEX.
    ...
    ImGuiColorEditFlags_Float =
            1 << 24,  // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ values
                      // formatted as 0.0f..1.0f floats instead of 0..255 integers. No round-trip of
                      // value via integers.
    ImGuiColorEditFlags_PickerHueBar =
            1 << 25,  // [PickerMode] // ColorPicker: bar for Hue, rectangle for Sat/Value.
    ImGuiColorEditFlags_PickerHueWheel =
            1 << 26,  // [PickerMode] // ColorPicker: wheel for Hue, triangle for Sat/Value.
...
struct ImGuiStyle {
    ...
    float PopupRounding;  // Radius of popup window corners rounding. (Note that tooltip windows use
                          // WindowRounding)
    float PopupBorderSize;  // Thickness of border around popup/tooltip windows. Generally set to
                            // 0.0f or 1.0f. (Other values are not well tested and more CPU/GPU
                            // costly).
    ...
    ImVec2 ButtonTextAlign;  // Alignment of button text when button is larger than text. Defaults
                             // to (0.5f,0.5f) for horizontally+vertically centered.
    ...
    ImVec2 DisplaySafeAreaPadding;  // If you cannot see the edges of your screen (e.g. on a TV)
                                    // increase the safe area padding. Apply to popups/tooltips as
                                    // well regular windows. NB: Prefer configuring your TV sets
                                    // correctly!
...
struct ImGuiIO {
    ...
    ImVec2 DisplaySize;   // <unset>          // Main display size, in pixels. For clamping windows
                          // positions.
    float DeltaTime;      // = 1.0f/60.0f     // Time elapsed since last frame, in seconds.
    float IniSavingRate;  // = 5.0f           // Minimum time between saving positions/sizes to .ini
                          // file, in seconds.
    const char* IniFilename;        // = "imgui.ini"    // Path to .ini file. Set NULL to disable
                                    // automatic .ini loading/saving, if e.g. you want to manually
                                    // load/save from memory.
    ...
    ImFont* FontDefault;        // = NULL           // Font to use on NewFrame(). Use NULL to uses
                                // Fonts->Fonts[0].
    ImVec2 DisplayFramebufferScale;  // = (1.0f,1.0f)    // For retina display or other situations
                                     // where window coordinates are different from framebuffer
                                     // coordinates. User storage only, presently not used by ImGui.
    ImVec2 DisplayVisibleMin;  // <unset>          // [OBSOLETE] If you use DisplaySize as a virtual
                               // space larger than your screen, set DisplayVisibleMin/Max to the
                               // visible area.
    ImVec2 DisplayVisibleMax;  // <unset>          // [OBSOLETE] Just use io.DisplaySize! If the
                               // values are the same, we defaults to Min=(0.0f) and Max=DisplaySize
    ...
    bool ConfigWindowsResizeFromEdges;  // = true           // Enable resizing of windows from their
                                        ...
                                        // the a per-window ImGuiWindowFlags_ResizeFromAnySide flag)
    ...
    bool NavVisible;  // Directional navigation is visible and allowed (will handle ImGuiKey_NavXXX
                      // events).
    float Framerate;  // Application framerate estimation, in frame per second. Solely for
                      // convenience. Rolling average estimation based on IO.DeltaTime over 120
                      // frames
    ...
    bool MouseDownOwned[5];  // Track if button was clicked inside a window. We don't request mouse
...
IMGUI_API bool InputFloat(
        ...
        ImGuiInputTextFlags flags =
                0);  // Use the 'const char* format' version instead of 'decimal_precision'!
...
static inline ImVec2 CalcItemRectClosestPoint(const ImVec2& pos, bool on_edge = false,
                                              float outward = 0.f) {
    (void)on_edge;
    (void)outward;
    IM_ASSERT(0);
...
typedef ImGuiInputTextCallback
        ImGuiTextEditCallback;  // OBSOLETE in 1.63 (from Aug 2018): made the names consistent
...
// Helper: IM_NEW(), IM_PLACEMENT_NEW(), IM_DELETE() macros to call MemAlloc + Placement New,
// Placement Delete + MemFree We call C++ constructor on own allocated memory via the placement
// "new(ptr) Type()" syntax. Defining a custom placement new() with a dummy parameter allows us to
// bypass including <new> which on some platforms complains when user has disabled exceptions.
struct ImNewDummy {};
...
// Helper: Macro for ImGuiOnceUponAFrame. Attention: The macro expands into 2 statement so make sure
// you don't use it within e.g. an if() statement without curly braces.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
// Helper: ImColor() implicity converts colors to either ImU32 (packed 4x1 byte) or ImVec4 (4x1
...
//-----------------------------------------------------------------------------
// Draw List API (ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel, ImDrawListFlags, ImDrawList,
// ImDrawData) Hold a series of drawing commands. The user provides a renderer for ImDrawData which
...
// for example to: A) Change your GPU render state, B) render a complex 3D scene inside a UI element
// without an intermediate texture/render target, etc. The expected behavior from your rendering
// function is 'if (cmd.UserCallback != NULL) { cmd.UserCallback(parent_list, cmd); } else {
// RenderTriangles() }'
...
// Draw channels are used by the Columns API to "split" the render list into different channels
// while building, so items of each column can be batched together. You can also use them to
// simulate drawing layers and submit primitives in a different order than how they will be
// rendered.
...
enum ImDrawListFlags_ {
    ...
    ImDrawListFlags_AntiAliasedLines =
            1 << 0,  // Lines are anti-aliased (*2 the number of triangles for 1.0f wide line,
...
// all command lists are passed to your ImGuiIO::RenderDrawListFn function for rendering. Each ImGui
// window contains its own ImDrawList. You can use ImGui::GetWindowDrawList() to access the current
// window draw list and draw custom primitives. You can interleave normal ImGui:: calls and adding
// primitives to the current draw list. All positions are generally in pixel coordinates (top-left
// at (0,0), bottom-right at io.DisplaySize), but you are totally free to apply whatever
// transformation matrix to want to the data (if you apply such transformation you'll want to apply
// it to ClipRect as well) Important: Primitives are always added to the list and not culled
// (culling is done at higher-level by ImGui:: functions), if you use this API a lot consider coarse
// culling your drawn objects.
struct ImDrawList {
    ...
    ImDrawListFlags Flags;  // Flags, you may poke into these to adjust anti-aliasing settings
                            // per-primitive.

    // [Internal, used while building lists]
    const ImDrawListSharedData*
            _Data;  // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to
    ...
    ImVector<ImDrawChannel>
            _Channels;  // [Internal] draw channels for columns API (not resized down so
                        // _ChannelsCount may be smaller than _Channels.Size)
    ...
    IMGUI_API void PushClipRect(
            ...
            bool intersect_with_current_clip_rect =
                    false);  // Render-level scissoring. This is passed down to your render function
    ...
    IMGUI_API void AddRect(
            ...
            float thickness = 1.0f);  // a: upper-left, b: lower-right, rounding_corners_flags:
    ...
    IMGUI_API void AddRectFilledMultiColor(const ImVec2& a, const ImVec2& b, ImU32 col_upr_left,
                                           ImU32 col_upr_right, ImU32 col_bot_right,
                                           ImU32 col_bot_left);
    IMGUI_API void AddQuad(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d,
                           ImU32 col, float thickness = 1.0f);
    IMGUI_API void AddQuadFilled(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d,
                                 ImU32 col);
    IMGUI_API void AddTriangle(const ImVec2& a, const ImVec2& b, const ImVec2& c, ImU32 col,
    ...
    IMGUI_API void AddImageQuad(ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b,
    ...
    IMGUI_API void AddImageRounded(ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b,
    ...
    IMGUI_API void AddBezierCurve(const ImVec2& pos0, const ImVec2& cp0, const ImVec2& cp1,
                                  const ImVec2& pos1, ImU32 col, float thickness,
                                  int num_segments = 0);
    ...
    IMGUI_API void
            AddDrawCmd();  // This is useful if you need to forcefully create a new draw call (to
                           // allow for dependent rendering / blending). Otherwise primitives are
                           // merged into the same draw-call as much as possible
...
// (NB: the style and the naming convention here is a little inconsistent but we preserve them for
...
struct ImDrawData {
    ...
    IMGUI_API void ScaleClipRects(
            const ImVec2& sc);  // Helper to scale the ClipRect field of each ImDrawCmd. Use if your
                                // final output buffer is at a different scale than ImGui expects,
                                // or if there is a difference between your window resolution and
...
struct ImFontConfig {
    ...
    int OversampleH;  // 3        // Rasterize at higher quality for sub-pixel positioning. We don't
...
struct ImFontGlyphRangesBuilder {
    ...
    ImFontGlyphRangesBuilder() {
        UsedChars.resize(0x10000 / 32);
        memset(UsedChars.Data, 0, 0x10000 / 32);
    }
    ...
    IMGUI_API void AddText(const char* text,
                           const char* text_end = NULL);  // Add string (each character of the UTF-8
                                                          // string are added)
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFont* AddFontFromMemoryTTF(
            ...
            const ImWchar* glyph_ranges =
                    NULL);  // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be
                            // deleted after destruction of the atlas. Set
                            // font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data
    ...
    IMGUI_API const ImWchar*
            GetGlyphRangesChineseFull();  // Default + Half-Width + Japanese Hiragana/Katakana +
    ...
    // After calling Build(), you can query the rectangle position and render your pixels. You can
    // also request your rectangles to be mapped as font glyph (given a font + Unicode point), so
    // you can render e.g. custom colorful icons and use them as regular glyphs.
    struct CustomRect {
        ...
        float GlyphAdvanceX;  // Input    // For custom font glyphs only (ID<0x10000): glyph
                              // xadvance
    ...
    IMGUI_API int AddCustomRectFontGlyph(
            ...
            const ImVec2& offset = ImVec2(0, 0));  // Id needs to be < 0x10000 to register a
    ...
    ImTextureID TexID;    // User data to refer to the texture once it has been uploaded to user's
...
struct ImFont {
    ...
    float FontSize;  // <user set>   // Height of characters, set during loading (don't change after
                     // loading)
    float Scale;  // = 1.f        // Base font scale, multiplied by the per-window font scale which
                  // you can adjust with SetFontScale()
    ImVec2 DisplayOffset;          // = (0.f,0.f)  // Offset font rendering by xx pixels
    ImVector<ImFontGlyph> Glyphs;  //              // All glyphs.
    ImVector<float>
            IndexAdvanceX;  //              // Sparse. Glyphs->AdvanceX in a directly indexable way
                            //              (more cache-friendly, for CalcTextSize functions which
    ...
    ImWchar FallbackChar;  // = '?'        // Replacement glyph if one isn't found. Only set via
                           // SetFallbackChar()

    // Members: Cold ~18/26 bytes
    short ConfigDataCount;     // ~ 1          // Number of ImFontConfig involved in creating this
                               // font. Bigger than 1 when merging multiple font sources into one
                               // ImFont.
    ImFontConfig* ConfigData;  //              // Pointer within ContainerAtlas->ConfigData
    ImFontAtlas* ContainerAtlas;  //              // What we has been loaded into
    float Ascent, Descent;        //              // Ascent: distance from top to bottom of e.g. 'A'
                                  //              [0..FontSize]
    bool DirtyLookupTables;
    int MetricsTotalSurface;  //              // Total surface in pixels to get an idea of the font
                              //              rasterization/texture cost (not exact, we approximate
    ...
    IMGUI_API void AddGlyph(ImWchar c, float x0, float y0, float x1, float y1, float u0, float v0,
