...
template <typename T,
          typename detail::types::enable_if<!detail::types::is_enum<T>::value, bool>::type = true>
...
template <typename T>
struct filldata<T*> {
    static void fill(std::ostream* stream, const T* in) { filldata<const void*>::fill(stream, in); }
};
}  // namespace detail
