...
WorldUpdate::WorldUpdate(std::map<int, Worm>* positions, std::map<int, WeaponDTO>* weapons): positions(positions), weapons(weapons) {}

char WorldUpdate::get_sent_by(ServerProtocol& prot) { return prot.send_WorldUpdate(*this); }

std::map<int, Worm>::const_iterator WorldUpdate::begin() const {
...
std::map<int, WeaponDTO>::const_iterator WorldUpdate::end_weapons() const { return this->weapons->end(); }
